function [V_out, new_vid] = add_vertices(V_set, V_in)
...
%
% - new_vid : positive integer row vector double, added vertices indices, size(new_vid) = [1,nb_new_vtx].
...
if isreal(V_set)        
    ...
    if ~isempty(V_set)
        ...
        new_vid = (1+size(V_in,1)):size(V_in,1)+size(V_set,1);
